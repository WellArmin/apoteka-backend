...
const mutations = {
   async createItem(parent, args, ctx, info){
    ...
    async resetPassword(parent, args, ctx, info){
         ...
         if(!user) {
             throw new Error('his token is wither invalid or expired');
         }
             const password = await bcrypt.hash(args.password, 10);
    ...
    async updatePermissions(parent, args, ctx, info){
        ...
        hasPermission(currentUser, ['ADMIN', 'PERMISSIONUPATE']);
...
*/
    ...
    async deleteCategory(parent, args, ctx, info){
        ...
        if(hasPermissions) {
            throw new Error('Nemate dozvolu da izbrišete ovu kategoriju!');
        }
    ...
    async createIntroSlider(parent, args, ctx, info){
        if(!ctx.request.userId){
            throw new Error('Niste prijavljeni kao administrator!');
        }
        const introSlider = ctx.db.mutation.createIntroSlider({
            data: {
                user: {
                    connect: {
                        id: ctx.request.userId,
    ...
    async deleteIntroSlider(parent, args, ctx, info){
        ...
        if(hasPermissions) {
            throw new Error('Nemate dozvolu da izbrišete ovaj slider!');
        }

        return ctx.db.mutation.deleteIntroSlider({ where}, info);
    },

    updateIntroSlider(parent, args, ctx, info){
        const updates = {...args};
        delete updates.id;
        return ctx.db.mutation.updateIntroSlider({
            data:updates,
            where: {
                id:args.id,
            },
